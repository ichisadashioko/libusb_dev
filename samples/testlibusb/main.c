      ...
      if (LIBUSB_DT_SS_ENDPOINT_COMPANION == endpoint->extra[i + 1])
        {
          ...
          ret = libusb_get_ss_endpoint_companion_descriptor (NULL, endpoint,
                                                             &ep_comp);
...
static void
print_device (libusb_device *dev, libusb_device_handle *handle)
{
  ...
  if (device_speed == LIBUSB_SPEED_LOW)
    {
      speed = "1.5M";
    }
  else if (device_speed == LIBUSB_SPEED_FULL)
    {
      speed = "12M";
    }
  else if (device_speed == LIBUSB_SPEED_HIGH)
    {
      speed = "480M";
    }
  else if (device_speed == LIBUSB_SPEED_SUPER)
    {
      speed = "5G";
    }
  else if (device_speed == LIBUSB_SPEED_SUPER_PLUS)
    {
      speed = "10G";
    }
  else
...
static int
test_wrapped_device (const char *device_name)
{
